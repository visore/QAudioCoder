...
int QLameCodec::encode(const qint8 input[], int inputSize)
{
	/*if(inputSize == 0)
	{
...
int QLameCodec::encode(const qint16 input[], int inputSize)
{
	QCodecChunk chunk = QCodecChunk::create(inputSize * 2);
	if(inputSize == 0)
	{
		chunk.setSize(m_lame_encode_flush(mLame, (unsigned char*) chunk.data(), chunk.size()));
	}
	else
	{
		//Input size is devided by 2, size Lame wants the samples per channel, not the samples in the array
		chunk.setSize(m_lame_encode_buffer_interleaved(mLame, (short int*) input, inputSize / 2, (unsigned char*) chunk.data(), chunk.size()));
		//return m_lame_encode_buffer_interleaved(mLame, (short int*) input, inputSize / 2, (unsigned char*) output, outputSize);
	}
	addChunk(chunk);
	return chunk.size();
...
int QLameCodec::encode(const qint32 input[], int inputSize)
{
	/*if(inputSize == 0)
	{
		return m_lame_encode_flush(mLame, (unsigned char*) output, outputSize);
